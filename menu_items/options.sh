...
render() {
  tmux display-menu -T "#[align=centre fg=green]Options" -x R -y P \
    ...
    "$(get_option_title show-fahrenheit)" B "run -b 'source #{kanagawaroot}/scripts/actions.sh toggle_option show-fahrenheit; tmux display-message \"Weather will be updated after next fetch.\"; source $CURRENT_FILE'" \
    "$(get_option_title military-time)" C "run -b 'source #{kanagawaroot}/scripts/actions.sh toggle_option military-time; source $CURRENT_FILE'" \
    "$(get_option_title show-flags)" D "run -b 'source #{kanagawaroot}/scripts/actions.sh toggle_option show-flags; source $CURRENT_FILE'" \
    "" \
